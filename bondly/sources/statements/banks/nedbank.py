...
def parse(full_text: str, first_page: str) -> StatementData:
    ...
    data.outstanding_balance = (
        extract_amount_near_label(r"Balance\s+Outstanding", full_text)
        or extract_amount_near_label(r"Outstanding\s+Balance", full_text)
    )
    data.original_loan_amount = extract_amount_near_label(
        r"(?:Original|Approved)\s+(?:Loan|Bond)\s+Amount", full_text
    )
    ...
    data.monthly_repayment = extract_amount_near_label(r"Monthly\s+Instalment", full_text)
    data.arrears_amount = (
        extract_amount_near_label(r"Arrears\s+Amount", full_text)
        or extract_amount_near_label(r"Arrears", full_text)
    )
    ...
    data.service_fee = extract_amount_near_label(
        r"(?:Service|Administration)\s+Fee", full_text
    )

    rows = []
    for m in re.finditer(
        r"(\d{2}\s+\w{3}\s+\d{4})\s+([\w\s/-]{5,50}?)\s{2,}"
