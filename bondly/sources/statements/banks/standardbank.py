...
def parse(full_text: str, first_page: str) -> StatementData:
    ...
    data.outstanding_balance = _extract_amount(
        r"(?:Loan|Outstanding|Current|Closing)\s+Balance[:\s]+R?\s*([\d\s,]+\.?\d{0,2})", full_text
    )
    data.original_loan_amount = _extract_amount(
        r"(?:Original\s+)?(?:Loan|Bond|Approved)\s+Amount[:\s]+R?\s*([\d\s,]+\.?\d{0,2})", full_text
    )
    ...
    data.monthly_repayment = _extract_amount(
        r"(?:Monthly\s+)?(?:Payment|Instalment|Repayment)[:\s]+R?\s*([\d\s,]+\.?\d{0,2})", full_text
    )
    data.arrears_amount = _extract_amount(
        r"(?:Overdue|Arrears)\s+(?:Amount)?[:\s]+R?\s*([\d\s,]+\.?\d{0,2})", full_text
    )
    ...
    data.service_fee = _extract_amount(
        r"(?:Service|Monthly\s+Admin)\s+Fee[:\s]+R?\s*([\d\s,]+\.?\d{0,2})", full_text
    )
